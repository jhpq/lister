...
class RdmsModel extends Model{
    ...
    function GetRequisiciones(){
        try{
            ...
            if ( $this->getSegmentParamValue( 0, '_search' ) && ($this->getSegmentParamValue( 0, '_search' )!='false') ){
                ...
                */
                ...
                if ( $no_unidad = $this->getSegmentParamValue( 0, 'no_unidad' ) ) {
                    $filter_clause .= " AND rm.unidad = '$no_unidad' ";
                }
                ...
                // Definir resto de filtros
                if ( $req_status = $this->getSegmentParamValue( 0, 'status_req' ) ) {

                    // Nuevos (abiertos sin no parte asignado)
                    if ($req_status==10){

                        //
                        $filter_status = 'GENERADAS';

                        $filter_clause .= "
                                            AND rest.codigo <> 'CANCELADOA%
                                            AND NOT EXISTS (SELECT * FROM rdm.dbo.rdm_articulos
                                            WHERE idrdm = rm.idrdm)
                                            ";
                    }

                    // Nuevos (vistos con no parte asignado)
                    if ($req_status==20){
                        //
                        $filter_status = 'PARTES ASIGNADAS';

                        $filter_clause .= "
                                            AND rest.codigo <> 'CANCELADA%'

                                            -- donde existan articulos anexados a la requisicion
                                            AND EXISTS (SELECT * FROM rdm.dbo.rdm_articulos
                                            WHERE idrdm = rm.idrdm)

                                            -- donde no tengan salidas
                                            AND NOT EXISTS(
                                                SELECT * FROM almacen.dbo.detallessalidas d
                                                LEFT JOIN almacen.dbo.articulos a ON a.idarticulo = d.idarticulo
                                                LEFT JOIN almacen.dbo.localizaciones loc ON loc.idlocalizacion = a.idlocalizacion
                                                WHERE d.idarticulo = rma.articulo_id
                                                AND rdm = rm.id
                                                AND loc.localizacion NOT LIKE '28-%')
                                                ";
                    }


                    // Requerido a proveedor es cuando se solicito un producto al proveedor
                    if ($req_status==30){
                        //
                        $filter_status = 'REQUERIDO A PROVEEDOR';
                    ...
                    if ($req_status==40){
                        ...
                        $filter_clause .= "
                                            ...
                                            -- donde existan ordenes de compra con el no. de requisicion
                                            AND EXISTS (SELECT * FROM compras.dbo.requisicionesdeordenescompra t
                                                LEFT JOIN compras.dbo.ordenescompra toc ON toc.ordencompra = t.ordencompra
                                                WHERE t.foliorequisicion = rm.idrdm)
                                                ";
                    }//--
                    ...
                    if ($req_status==80){
                        //
                        $filter_status = 'CANCELADAS';
                        $filter_clause .= "
                                            AND rm.estatus = 'CANCELADA%'
                                           ";
                    }
                            ...
                            (SELECT
                                ...
                                FROM
                                    (SELECT
                                        ...
                                        ,CONVERT(CHAR,rest.fecha,21) As fecha_requisicion
                                        ...
                                        FROM rdm.dbo.rdm rm
                                            ...
                                            WHERE rm.depto_id = $departamento_id
                                            $filter_clause
                                            ) t
                                        )t
                                ";*/
            ...
            $sql_total = "
                        SELECT COUNT(*)as total FROM (SELECT rm.idrdm as id, rm.eco as unidad, est.name as prev_status, rest.fecha as fecha_estatus  
                        FROM rdm.dbo.rdm rm 
            ...
            */
            ...
            /* Get Table Fields */
            $table_fields = "rm.idrdm as id, rm.eco as unidad, no_oc, no_factura, datetime_prioridad_requisicion, fecha_visto, descripcion, concepto_id, concepto_tipo, incluye_muestra, observacion, prev_status, fecha_requisicion, usuario_id, usuario_nombre, sucursal_id, sucursal_shortname, pc, usuario_modifica_id, usuario_modifica_nombre, sucursal_modifica_id, sucursal_modifica_shortname, mecanico_id, mecanico_nombre";
           ...
           /* $scl_clause = "
                    ...
                    SELECT
                       idrdm as id , eco as unidad, datetime_prioridad_requisicion, descripcion, concepto_id, concepto_tipo, incluye_muestra, observacion, prev_status, fecha_requisicion
                        FROM
                            (SELECT
                                 ROW_NUMBER() OVER ( ORDER BY idrdm desc) as row
                                ,*
                                FROM
                                    (SELECT
                                        ...
                                        ,CONVERT(CHAR,rdes.fecha,21) As fecha_requisicion
                                        ...
                                        FROM rdm.dbo.rdm rm
                                            ...
                                            LEFT JOIN rdm.dbo.rdm_articulos rma ON rm.idrdm = rma.idrdm
                                            --
                                            LEFT JOIN rdm.dbo.rdm_detalles rdmrd ON rdmrd.idrdm = rm.idrdm
                                            LEFT JOIN rdm.dbo.rdm_detalles_Estatus rdes ON rdes.idrdm_detalles = rdmrd.idrdm_detalles
                                            LEFT JOIN rdm.dbo.conceptos_rdm tc  ON tc.concepto_id = rm.concepto_id
                                            LEFT JOIN rdm.dbo.rdm_estatus rdmest ON rdmest.idrdm = rm.idrdm  
                                            LEFT JOIN rdm.dbo.estatus est ON est.codigo= rdmest.codigo
                                            WHERE rm.depto_id  = $departamento_id
                                            $filter_clause
                                            ) t
                                        )t
                                        WHERE 1=1 AND year(fecha_requisicion) = '2014' ORDER BY fecha_requisicion DESC
                                ";*/

            $scl_clause = "
                                   
                        SELECT rm.idrdm as id, rm.eco as unidad, est.name as prev_status, CONVERT(CHAR, rest.fecha, 21) as fecha_estatus , CONVERT(CHAR, rm.prioridad, 21) as datetime_prioridad_requisicion, con.tipo as concepto_tipo
                        FROM rdm.dbo.rdm rm 
                        LEFT JOIN rdm.dbo.rdm_estatus rest ON rest.idrdm = rm.idrdm and rest.id_rdm_estatus = (select max(rest.id_rdm_estatus) from rdm.dbo.rdm_estatus rest where rest.idrdm = rm.idrdm)
                        LEFT JOIN rdm.dbo.estatus est ON  est.codigo = rest.codigo
                        LEFT JOIN rdm.dbo.conceptos_rdm  con ON con.concepto_id = rm.concepto_id 
                        WHERE rm.depto_id = $departamento_id
                        AND year(rest.fecha) = '2014' ORDER by rm.idrdm DESC
            ";                    
    ...
    function GetRequisicionDetalles(){
        try{
            ...
            if ($rid = $this->getSegmentValue(1)){
                ...
                $sql_clause="
                    SELECT  rdet.idrdm as id ,rdet.idrdm_detalles, rdet.numeroparte, rdet.cantidad, rdet.descripcion, est.name as estatus, CONVERT(CHAR,rdest.fecha,21) as fecha_estatus FROM rdm.dbo.rdm_detalles rdet
